...
//         char buf[256];
//         sprintf(buf, "\tmovq %d(`s0), `d0 # spilled use %d", acc->u.offset, Temp_getTempnum(t));
...
//         Temp_temp newTemp = Temp_newtemp();
//         char buf[256];
//         sprintf(buf, "\tmovq `s0, %d(`s1) # spilled def %d", acc->u.offset, Temp_getTempnum(t));
